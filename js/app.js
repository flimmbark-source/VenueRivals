...
(function () {
    ...
    function syncDeckInventoryWithDeck() {
        const inventoryCounts = createNeutralInventoryCounts();
        loadoutState.deck.forEach((guestId) => {
            if (typeof inventoryCounts[guestId] === 'number' && inventoryCounts[guestId] > 0) {
                inventoryCounts[guestId] -= 1;
            }
        });
        loadoutState.deckInventoryCounts = inventoryCounts;
    }
    ...
    function renderDeckManage() {
        ...
        if (inspectMode && loadoutState.deck.length >= DECK_INSPECT_MAX_SIZE) {
    ...
    function onDeckInventoryDrop(event) {
        ...
        if (Number.isInteger(data.index) && data.index >= 0 && data.index < loadoutState.deck.length) {
            ...
            if (removedGuestId && typeof loadoutState.deckInventoryCounts[removedGuestId] === 'number') {
                loadoutState.deckInventoryCounts[removedGuestId] += 1;
            }
            renderDeckManage();
            renderLoadoutDeck();
            checkStartEnabled();
